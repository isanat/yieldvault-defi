...
import { Loader2, Filter, Search, Clock, User, Activity, RefreshCw } from 'lucide-react';
...
const ACTION_LABELS: Record<string, { label: string; color: string }> = {
  ...
  deposit: { label: 'Deposit', color: 'text-green-400 bg-green-500/20' },
  withdraw: { label: 'Withdraw', color: 'text-orange-400 bg-orange-500/20' },
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-800/50 rounded-xl border border-slate-700 p-4">
          <p className="text-gray-400 text-sm">Features Toggles</p>
          <p className="text-2xl font-bold text-blue-400 mt-1">
            {logs.filter(l => l.action === 'toggleFeature').length}
          </p>
        </div>
      </div>
    </div>
